...
class KspaceWgan(BasicModel):
    ...
    def __G__(self):
        ...
        if self.FLAGS.dump_debug:
            image_with_zero_padding = self.get_reconstructed_image(
                real=self.input['real'][:, 1, :, :] * self.input['mask_2'],
                imag=self.input['imag'][:, 1, :, :] * self.input['mask_2'], name='NoisePadding')
        ...
        up2 = tf.concat([up3[:, :, :-1, :], conv3], axis=1, name="concat_2k")
        out_dim = 128
        ...
        up1 = tf.concat([up2[:, :, :, :-1], conv2], axis=1, name="concat_1k")
        out_dim = 64
        ...
        up0 = tf.concat([up1[:, :, :-1, :-1], conv1], axis=1, name="concat_0k")
        out_dim = 64
        ...
        up2 = tf.concat([up3[:, :, :-1, :], conv3], axis=1, name="concat_2")
        out_dim = 128
        up2 = tf.layers.conv2d_transpose(up2, out_dim, [4, 4], [2, 2], padding='same', activation=None,
        ...
        up1 = tf.concat([up2[:, :, :, :-1], conv2], axis=1, name="concat_1")
        out_dim = 64
        up1 = tf.layers.conv2d_transpose(up1, out_dim, [4, 4], [2, 2], padding='same', activation=None,
        ...
        up0 = tf.concat([up1[:, :, :-1, :-1], conv1], axis=1, name="concat_0")
        out_dim = 64
        up0 = tf.layers.conv2d_transpose(up0, out_dim, [4, 4], [2, 2], padding='same', activation=None,
        ...
        # Dump prediction out
        tf.summary.image('G_predict_diff', tf.transpose(conv_last, (0, 2, 3, 1)), collections='G', max_outputs=4)
    ...
    def __loss__(self):
        ...
        with tf.variable_scope("discriminator") as scope:
            self.d_loss_real = tf.losses.sigmoid_cross_entropy(
                tf.ones_like(self.predict_d_logits) + tf.random_normal(tf.shape(self.predict_d_logits), 0, 0.1),
                self.predict_d_logits)
            tf.summary.scalar('d_loss_real', self.d_loss_real, collections='D')
            # scope.reuse_variables()
            self.d_loss_fake = tf.losses.sigmoid_cross_entropy(tf.zeros_like(self.predict_d_logits_for_g),
                                                               self.predict_d_logits_for_g)
