...
class KspaceWgan(BasicModel):
    ...
    def __init__(self,real,imag,input=None, dims_in=None, dims_out=None, FLAGS=None, train_phase=None, adv_loss_w=None,epoch=None):
        """
        :param input:
        :param labels:
        :param dims_in:
    ...
    def __training__(self, learning_rate):
        ...
        """
        decay = tf.floordiv(self.epoch,10)

        learning_rate = self.FLAGS.learning_rate*tf.pow(2.,-decay)
        ...
        optimizer_u = tf.train.AdamOptimizer(learning_rate)
        optimizer_k = tf.train.AdamOptimizer(learning_rate)
