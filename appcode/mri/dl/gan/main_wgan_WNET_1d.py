...
from appcode.mri.dl.gan.k_space_wgan_WNET_1d import KspaceWgan
...
flags.DEFINE_integer('iters_no_adv', 10000, 'Iters with adv_w=0')
...
def run_evaluation_all(sess, feed, net):
    """
...
def train_model(mode, checkpoint=None):
    ...
    val_datset = whole_dataset.skip(31360)
    train_dataset = whole_dataset.take(31360)
    ...
    # Shuffle the dataset
    tf.set_random_seed(50)
    val_datset = val_datset.shuffle(buffer_size=1000)
    # Repeat the input indefinitly
    ...
    for i in range(start_iter, FLAGS.max_steps):
        ...
        # try:
        if i % FLAGS.print_test == 0 and  i>0:
            # Record summary data and the accuracy
            if feed_val is not None:
                run_evaluation_all(sess, feed_val, net=net)
...
def evaluate_checkpoint(tt='test', checkpoint=None, output_file=None, output_file_interp=None):
    ...
    """
    # Import data
    data_set = KspaceDataSet(base_dir, file_names.values(), stack_size=32, shuffle=False, data_base=FLAGS.database)
