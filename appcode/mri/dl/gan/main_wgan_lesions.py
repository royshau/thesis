...
from appcode.mri.dl.gan.k_space_wgan_lesions import KspaceWgan
...
def _parse_(serialized_example):
    ...
    real = tf.reshape(real, [3, 182, 218])
    imag = tf.decode_raw(example['imag'], tf.float32)
    imag = tf.reshape(imag, [3, 182, 218])
    return real, imag
...
tfrecords = {'test': ["/HOME/data2/LesionsBig_test.tfrecords"], 'train': ["/media/rrtammyfs/Projects/2018/MRIGAN/Data_tfrecords/LesionsPP_train.tfrecords"]}
...
flags.DEFINE_integer('iters_no_adv', 5000, 'Iters with adv_w=0')
...
flags.DEFINE_string('database', 'Lesions-PP', "data base name - for file info")
flags.DEFINE_boolean('dump_debug', True, 'wide_debug_tensorboard')
keep_center = 0.05
DIMS_IN = np.array([3, 182, 218])
DIMS_OUT = np.array([1, 182, 218])
...
mask_1 = masks['mask_1'][37:256-37,19:256-19]
mask_2 = masks['mask_2'][37:256-37,19:256-19]
mask_3 = masks['mask_3'][37:256-37,19:256-19]
