...
class KspaceWgan(BasicModel):
    ...
    def __G__(self):
        ...
        predict_kspace = {}
        predict_kspace['real'] = tf.reshape(self.conv_last_kspace[:, 0, :, :],
                                            [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]],
                                            name='G_predict_real')
        predict_kspace['imag'] = tf.reshape(self.conv_last_kspace[:, 1, :, :],
                                            [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]],
                                            name='G_predict_imag')
    ...
    def __D__(self, input_d, predict_g, input_type):
        ...
        # Model convolutions
        out_dim = 64  # 128x128
        self.conv_1_d = ops.conv2d(input_to_discriminator, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1,
                                   name="D_conv_1")
    ...
    def __loss__(self):
        ...
        with tf.variable_scope("discriminator") as scope:
            # self.d_loss_real = tf.losses.sigmoid_cross_entropy(self.predict_d_logits,tf.ones_like(self.predict_d_logits)+tf.random_normal(tf.shape(self.predict_d_logits),0,0.1))
            # tf.summary.scalar('d_loss_real', self.d_loss_real, collections='D')
            # # scope.reuse_variables()
            # self.d_loss_fake = tf.losses.sigmoid_cross_entropy(self.predict_d_logits_for_g,tf.zeros_like(self.predict_d_logits_for_g))
        ...
        # Generative loss
        # g_loss = tf.losses.sigmoid_cross_entropy(self.predict_d_logits_for_g, tf.ones_like(self.predict_d_logits_for_g))
        ...
        self.context_loss_total = self.FLAGS.gen_loss_context * self.context_loss + self.FLAGS.im_loss_context * self.im_loss + self.FLAGS.im_loss_context_l1 * self.l1_loss
