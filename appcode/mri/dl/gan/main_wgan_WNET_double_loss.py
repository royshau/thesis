...
from appcode.mri.dl.gan.k_space_wgan_WNET_double_loss import KspaceWgan
...
flags.DEFINE_float('learning_rate', 0.000075, 'Initial learning rate.')
...
flags.DEFINE_integer('mini_batch_predict', 32, 'Size of mini batch for predict')
...
flags.DEFINE_float('final_gen_loss_context', 0.001, 'Generative loss, kspace context weight.')
flags.DEFINE_float('im_loss_context', 10, 'Generative loss, Image context weight.')
flags.DEFINE_float('first_im_loss_context', 1, 'Generative loss, Image context weight.')
flags.DEFINE_float('im_loss_context_l1', 0, 'Generative loss, Image context weight.')
...
flags.DEFINE_integer('iters_no_adv', 105000, 'Iters with adv_w=0')
flags.DEFINE_integer('train_D_every', 1000000, 'Long Train D every N iterataions')
flags.DEFINE_integer('num_D_updates_long', 500, 'Discriminator update freq for long train')
...
def run_evaluation(sess, feed, net, step, writer, tt):
    ...
    r_g, r_d, loss_d_fake, loss_d_real, loss_d, loss_g, l2_norm, g_loss_no_reg, d_loss_no_reg, g_loss_im, g_loss_l1, psnr,kspace_final,im_loss_first = sess.run(
        [m_op_g, m_op_d, net.d_loss_fake, net.d_loss_real,
         ...
         net.psnr,net.final_context_loss,net.first_im_loss], feed_dict=feed)

    writer['G'].add_summary(r_g, step)
    writer['D'].add_summary(r_d, step)

    print('%s:  Time: %s , Loss at step %s: D: %s, G: %s,  L2: %s / %s, L2_Im: %s / %s L1: %s PSNR: %s' % (
    tt, datetime.datetime.now(), step, d_loss_no_reg, loss_g, l2_norm,kspace_final,im_loss_first,g_loss_im, g_loss_l1, psnr))
...
def train_model(mode, checkpoint=None):
    ...
    feed_test = feed_data(0, net.labels, net.train_phase,
                          tt='test')
    feed_test[handle] = test_handle
    feed_test[net.adv_loss_w] = gen_loss_adversarial
    ...
    for i in range(start_iter, FLAGS.max_steps):
        ...
        if i % FLAGS.print_test == 0:
            ...
            if feed_test is not None:
                run_evaluation(sess, feed_test, step=i, net=net, writer=writer['test'], tt='TEST')
            ...
            for it in np.arange(num_D_updates):
                if feed is not None and i > 1000:
