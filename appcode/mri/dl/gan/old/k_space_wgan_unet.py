...
class KspaceWgan(BasicModel):
    ...
    def __G__(self):
        ...
        predict_kspace = {}
        predict_kspace['real'] = tf.reshape(self.conv_6[:,0,:,:], [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]], name='G_predict_real')
        predict_kspace['imag'] = tf.reshape(self.conv_6[:,1,:,:], [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]], name='G_predict_imag')
        ...
        image_space = (image_space - tf.reduce_min(image_space))/(tf.reduce_max(image_space)-tf.reduce_min(image_space))
    ...
    def __D__(self, input_d,predict_g, input_type):
        ...
        org = (org - tf.reduce_min(org)) / (tf.reduce_max(org) - tf.reduce_min(org))
...
def normalize(tensor):
    ...
    std = tf.sqrt(var)
    norm_tensor = (tf.nn.bias_add(tensor,-mean))/std
    return norm_tensor,mean,std

def denormalize(tensor,mean,std):
    tensor = tensor*std
    tensor = tf.nn.bias_add(tensor,mean)
    return tensor