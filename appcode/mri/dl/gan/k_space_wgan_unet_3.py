...
class KspaceWgan(BasicModel):
    ...
    def __init__(self, real, imag, input=None, dims_in=None, dims_out=None, FLAGS=None, train_phase=None,
                 adv_loss_w=None):
        """
        :param input:
        :param labels:
    ...
    def build(self):
        ...
        with tf.name_scope('training'):
            self.train_op_d, self.train_op_g, self.train_op_u, self.train_op_k = self.__training__(
    ...
    def __G__(self):
        ...
        if self.FLAGS.dump_debug:
            tf.summary.image('G_mask', tf.transpose(self.labels['mask_2'], (0, 2, 3, 1)), collections='G',
                             max_outputs=1)
            ...
            image_with_zero_padding = self.get_reconstructed_image(real=self.input['real'] * self.input['mask_2'],
                                                                   imag=self.input['imag'] * self.input['mask_2'],
                                                                   name='NoisePadding')
        ...
        self.conv_1 = ops.conv2d(x_input_stack, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_1")
        self.conv_1_bn = ops.batch_norm(self.conv_1, self.train_phase, decay=0.98, name="G_K_bn1")
        self.relu_1 = tf.nn.relu(self.conv_1_bn)

        out_dim = 32

        self.conv_1_1 = ops.conv2d(self.relu_1, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_1.1")
        self.conv_1_1_bn = ops.batch_norm(self.conv_1_1, self.train_phase, decay=0.98, name="G_K_bn1_1")
        ...
        out_dim = 32
        self.conv_2 = ops.conv2d(self.relu_1_1, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_2")
        self.conv_2_bn = ops.batch_norm(self.conv_2, self.train_phase, decay=0.98, name="G_K_bn2")
        self.relu_2 = tf.nn.relu(self.conv_2_bn)

        out_dim = 64
        self.conv_3 = ops.conv2d(self.relu_2, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_3")
        self.conv_3_bn = ops.batch_norm(self.conv_3, self.train_phase, decay=0.98, name="G_K_bn3")
        self.relu_3 = tf.nn.relu(self.conv_3_bn)

        out_dim = 32
        self.conv_4 = ops.conv2d(self.relu_3, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_4")
        self.conv_4_bn = ops.batch_norm(self.conv_4, self.train_phase, decay=0.98, name="G_K_bn4")
        self.relu_4 = tf.nn.relu(self.conv_4_bn)

        out_dim = 8
        self.conv_5 = ops.conv2d(self.relu_4, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_5")
        self.conv_5_bn = ops.batch_norm(self.conv_5, self.train_phase, decay=0.98, name="G_K_bn5")
        self.relu_5 = tf.nn.relu(self.conv_5_bn)

        out_dim = 2
        self.conv_6 = ops.conv2d(self.relu_5, output_dim=out_dim, k_h=3, k_w=3, d_h=1, d_w=1, name="G_K_conv_6")

        # Denorm

        predict_kspace = {}
        predict_kspace['real'] = tf.reshape(self.conv_6[:, 0, :, :],
                                            [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]],
                                            name='G_predict_real')
        predict_kspace['imag'] = tf.reshape(self.conv_6[:, 1, :, :],
                                            [-1, self.dims_out[0], self.dims_out[1], self.dims_out[2]],
                                            name='G_predict_imag')
        ...
        # Get image space:
        image_space = self.get_reconstructed_image(real=predict_kspace['real'], imag=predict_kspace['imag'],
                                                   name="predict_g1")
        image_space = tf.abs(tf.complex(real=tf.expand_dims(image_space[:, 0, :, :], axis=1),
                                        imag=tf.expand_dims(image_space[:, 1, :, :], axis=1)))

        image_space = (image_space - tf.reduce_min(image_space)) / (
                    tf.reduce_max(image_space) - tf.reduce_min(image_space)) * 2 - 1
        ...
        up4 = tf.layers.conv2d_transpose(up4, out_dim, [4, 4], [2, 2], padding='same', activation=None,
                                         data_format="channels_first", name='G_U_conv4')
        up4 = tf.nn.relu(tf.layers.batch_normalization(up4, training=self.train_phase, axis=1, name='G_U_conv4_bn'),
                         name='G_U_conv4_act')
        ...
        up3 = tf.layers.conv2d_transpose(up3, out_dim, [4, 4], [2, 2], padding='same', activation=None,
                                         data_format="channels_first", name='G_U_conv5')
        up3 = tf.nn.relu(tf.layers.batch_normalization(up3, training=self.train_phase, axis=1, name='G_U_conv5_bn'),
                         name='G_U_conv5_act')
        ...
        up2 = tf.layers.conv2d_transpose(up2, out_dim, [4, 4], [2, 2], padding='same', activation=None,
                                         data_format="channels_first", name='G_U_conv6')
        up2 = tf.nn.relu(tf.layers.batch_normalization(up2, training=self.train_phase, axis=1, name='G_U_conv6_bn'),
                         name='G_U_conv6_act')
        ...
        up1 = tf.layers.conv2d_transpose(up1, out_dim, [4, 4], [2, 2], padding='same', activation=None,
                                         data_format="channels_first", name='G_U_conv7')
        up1 = tf.nn.relu(tf.layers.batch_normalization(up1, training=self.train_phase, axis=1, name='G_U_conv7_bn'),
                         name='G_U_conv7_act')
        ...
        up0 = tf.layers.conv2d_transpose(up0, out_dim, [4, 4], [2, 2], padding='same', activation=None,
                                         data_format="channels_first", name='G_U_conv8')
        up0 = tf.nn.relu(tf.layers.batch_normalization(up0, training=self.train_phase, axis=1, name='G_U_conv8_bn'),
                         name='G_U_conv8_act')
        ...
        # Dump prediction out
        tf.summary.image('G_predict_diff', tf.transpose(pred_image - image_space, (0, 2, 3, 1)), collections='G',
                         max_outputs=4)
        tf.summary.image('G_predict_final', tf.transpose(predict['image'], (0, 2, 3, 1)), collections='G',
                         max_outputs=4)
    ...
    def __loss__(self):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            if self.FLAGS.dump_debug:
                tf.summary.image('D_predict_real',
                                 tf.transpose(tf.reshape(self.predict_d_logits, (-1, 1, 1, 1)), (0, 2, 3, 1)),
                                 collections='D')
                tf.summary.image('D_predict_fake',
                                 tf.transpose(tf.reshape(self.predict_d_logits_for_g, (-1, 1, 1, 1)), (0, 2, 3, 1)),
                                 collections='D')
        ...
        real_diff = tf.contrib.layers.flatten(
            tf.multiply(self.predict_g_kspace['real'] - self.input_d['real'], mask_not))
        imag_diff = tf.contrib.layers.flatten(
            tf.multiply(self.predict_g_kspace['imag'] - self.input_d['imag'], mask_not))
