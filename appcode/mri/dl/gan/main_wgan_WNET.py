...
from appcode.mri.dl.gan.k_space_wgan_WNET import KspaceWgan
...
flags.DEFINE_float('learning_rate', 0.00075, 'Initial learning rate.')
...
flags.DEFINE_integer('iters_no_adv', 2000000, 'Iters with adv_w=0')
flags.DEFINE_integer('train_D_every', 165000, 'Long Train D every N iterataions')
flags.DEFINE_integer('num_D_updates_long', 1000, 'Discriminator update freq for long train')

flags.DEFINE_integer('dataset_size', 39200, 'Total train set size(including val)')
flags.DEFINE_float('val_perc', 0.2, 'Validation set ratio of train set')
...
def run_evaluation_all(sess, feed, net, step, writer, tt):
    ...
    print("Total accuracy for Val set is kspace_l2: %s PSNR: %s NMSE %s" % (
...
def load_graph(iterator):
    ...
    epoch = tf.placeholder(tf.float32, name='context_loss_w')
    adv_loss_w = tf.placeholder(tf.float32, name='adv_loss_w')

    network = KspaceWgan(real, imag, input=y_input, dims_in=DIMS_IN,
                         dims_out=DIMS_OUT, FLAGS=FLAGS, train_phase=train_phase, adv_loss_w=adv_loss_w,epoch=epoch)
    network.build()
    return network
...
def train_model(mode, checkpoint=None):
    ...
    train_size = np.round(FLAGS.dataset_size*(1-FLAGS.val_perc))
    steps_per_epoch = np.round(train_size/FLAGS.mini_batch_size)
    print("{} train samples.".format(train_size))

    val_datset = whole_dataset.skip(train_size)
    train_dataset = whole_dataset.take(train_size)
    # Shuffle the dataset
    ...
    for i in range(start_iter, FLAGS.max_steps):
        epoch = np.round(i / steps_per_epoch)
        feed[net.epoch] = epoch
        ...
        if i % steps_per_epoch == 0 and  i>0:
            ...
            gen_loss_adversarial = FLAGS.gen_loss_adversarial+epoch*0.01
            feed[net.adv_loss_w] = gen_loss_adversarial

            print("Epoch = {} , New gen loss = {}".format(epoch,gen_loss_adversarial))
        else:
            # Training
            # Update D network
